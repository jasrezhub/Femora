...
// Firebase URL
var FB = 'https://femora-default-rtdb.firebaseio.com';
...
function getCart() { return JSON.parse(localStorage.getItem('femora_cart') || '[]'); }
function saveCart(c) {
  localStorage.setItem('femora_cart', JSON.stringify(c));
  updateCartCount();
}
...
function initAgeGate() {
  if (sessionStorage.getItem('femora_age_verified') === '1') {
    var g = document.getElementById('age-gate');
    if (g) g.style.display = 'none';
  }
}
function ageGateConfirm() {
  sessionStorage.setItem('femora_age_verified','1');
  var g = document.getElementById('age-gate');
